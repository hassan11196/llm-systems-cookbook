...
- **Mercury 2** (Inception): a production diffusion-based LLM that generates tokens in parallel rather than auto-regressively, achieving speeds above 1,000 tokens/sec. Challenges the assumption that all LLM inference is sequential; relevant to the roofline analysis in chapters 1–2 as an alternative compute model.
...
- **Cloudflare Infire**: Cloudflare's custom inference engine that runs LLMs across multiple GPUs more efficiently, reduces memory usage, and starts models more quickly — ships alongside **Unweight**, a weight compression system that reduces LLM model sizes 15–22% without measurable accuracy loss.

...
- **MCP 2026-07-28 spec (release candidate)**: the largest MCP revision since launch — stateless protocol core (no `Mcp-Session-Id`), `initialize`/`initialized` handshake removed, authorization rewritten around OAuth/OIDC RFCs, and a new reverse-DNS extensions framework carrying the Tasks extension and MCP Apps. Still in release-candidate status as of mid-July 2026; the final text ships July 28, 2026. The Enterprise-Managed Authorization extension has already reached stable status (adopted by Anthropic, Microsoft, Okta) ahead of the core spec. Notebook 04_agents/05 (MCP server + client) and 08_production/06 (real MCP server) should migrate their handshake and session-handling code off the pre-2026-07-28 assumptions once the spec finalizes.

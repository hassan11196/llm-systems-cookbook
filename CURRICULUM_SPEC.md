...
**Framework version pins (last validated June 2026; pins in `pyproject.toml` and `environment.yml` are authoritative and may drift as the ecosystem moves):** `torch==2.7.*` (2.9 for FSDP2 notebooks), `triton==3.3.*`, `transformers==4.52.*` (RAG notebooks pin lower for `sentence-transformers` / `pylate` compatibility — see Spec-level risks), `vllm==0.22.*` (v0.22.1, June 2026; Model Runner V2 — enabled via `VLLM_USE_V2_MODEL_RUNNER=1` — replaces CPU PyTorch ops with GPU-native Triton kernels, delivering 56% higher throughput on GB200; v0.22 adds Rust frontend preview, 28.9% latency gain via Cutlass FP8, multi-tier KV offload, DeepSeek V4 Pro/Flash NVFP4 fused MoE; v0.22.1 adds JetBrains Mellum v2 and zentorch AMD CPU quant; requires C++20 build toolchain; Transformers v4 deprecated in v0.22; NVIDIA Dynamo backend available as optional extra), `sglang==0.5.*` (v0.5.12.post1, May 26 2026; HiCache UnifiedRadixTree + SSD offload via Mooncake, EAGLE-3 speculative decoding, DeepSeek V4 / Ring-2.6-1T / Gemma 4 support; post1 restores B200/B300 DeepSeek V4 accuracy; XGrammar-2 ~80× faster grammar compilation and ~3× faster constrained decoding vs vLLM; achieved 25× inference performance on GB300 NVL72 in Feb 2026), `xgrammar==0.2.*` (XGrammar-2, May 4 2026; Structural Tag protocol unifies tool calling, reasoning channels, and custom output formats; integrated into vLLM, SGLang, and TensorRT-LLM), `sentence-transformers==5.4.1`, `faiss-cpu==1.13.2`, `peft==0.14.*` (DoRA supported ≥ 0.10), `trl==0.26.*`, `bitsandbytes==0.45.*`, `llmcompressor==0.10.*`, `langgraph==1.4.*`, `dspy-ai==3.0.*`, `autogen-agentchat==0.4.*` (Microsoft merged AutoGen with Semantic Kernel into **Microsoft Agent Framework 1.0** in April 2026; AutoGen 0.4 enters maintenance mode; community fork continues as AG2 at `ag2ai/ag2`), `microsoft-agent-framework==1.0.*` (production successor to AutoGen 0.4 + Semantic Kernel), `crewai==1.12.*`, `mcp==1.6.*` (MCP 2026 spec RC published May 2026; final spec July 28 2026 — stateless Streamable HTTP core, Tasks extension, MCP Apps), `openai-agents==0.1.*` (OpenAI Agents SDK, March 2025), `google-adk==1.0.*`, `pydantic-ai==0.1.*`, `lm-eval==0.4.15`, `inspect-ai==0.3.210`, `lighteval==0.11.0`, `ragas==0.2.14` (API-stable; 0.4.x available but breaks the 0.2.x metric API used in 02_rag/09 — upgrade is a v0.2 stretch goal), `jax==0.6.*`. **Note:** HuggingFace TGI moved to maintenance mode in 2025; vLLM and SGLang are the recommended replacements for production inference serving.
...
The dominant architecture shift since v0.1 is that MoE is now the default at scale: DeepSeek V4 Pro (1.6T total / 49B active, 1M-token context), Llama 4 Maverick (400B / 17B active, MoE-first in the Llama line), Qwen3.5-397B-A17B (flagship reasoning/math MoE), and Gemma 4 (multimodal, April 2026) all use sparse MoE. DeepSeek V4 Flash (284B / 13B active) is a cost-efficient open MoE option for demo notebooks requiring larger active param counts than Qwen3-0.6B. Notable frontier-class open models released in April–June 2026 that further close the open/closed gap: **Kimi K2.6** (Moonshot AI; 1T/32B active MoE, 80.2% SWE-bench Verified, Agent Swarm system scaling to 300 domain-specialized sub-agents across 4,000 coordinated steps), **Qwen 3.6 Plus** (Alibaba; 1M-token context, 78.8% SWE-bench Verified, leads Terminal-Bench 2.0), **GLM-5.1** (Z.AI; 754B/MoE, 58.4% SWE-bench Pro), **MiMo-V2.5** (Xiaomi; 310B/15B active MoE, 48T tokens, 1M-token context, fully open-sourced on HuggingFace, frontier-level multimodal agentic capability), and **MiniMax M3** (MiniMax, June 1 2026; first open-weight model combining frontier coding + 1M context + native multimodal input; MSA sparse attention, 59.0% SWE-bench Pro, 70.06% OSWorld-Verified; open weights expected ~June 10–11 2026). **DeepSeek R1-0528** (May 28 2026) is a major reasoning upgrade: AIME 2025 70% → 87.5%, 2× deeper thinking (23K vs 12K avg tokens), 45–50% hallucination reduction. Apache 2.0 has become the dominant permissive license for 2026 open models (Gemma 4, Qwen 3.5, Mistral Large 3, Kimi K2.6); DeepSeek V4 ships MIT.
...
- **VeriCache** (arXiv 2605.17613, May 2026): Compressed KV drafts tokens while full-precision KV on CPU/disk verifies — achieves lossless output quality at compressed-cache throughput. Natural extension to the KV compression notebooks (05_serving/03, 05_serving/04) and the speculative decoding notebook (01_inference/07).
- **NVIDIA Vera Rubin platform** (announced GTC 2026): Rubin GPU (336B transistors, 288 GB HBM4, 50 PFLOPS FP4), Vera CPU (72 ARM Grace cores), NVLink 6 interconnect, targeting 5× Blackwell inference throughput at 10× lower token cost. Partner availability (AWS, GCP, Azure, CoreWeave, Lambda) targeted H2 2026. The GPU pricing notebook (08_production/09) should be updated when Rubin SKUs and pricing are published.
- **MCP 2026 specification RC** (May 2026, final July 28 2026): stateless Streamable HTTP core, SEP-2549 TTL caching, Tasks extension, MCP Apps, OAuth/OIDC hardening. The MCP server/client notebook (04_agents/05 and 08_production/06) should be updated to reference the RC spec; the final spec may require minor API changes before July.
- **MiniMax M3 / DeepSeek R1-0528** (June 2026): New open-weight models to document in the model registry and GPU pricing notebook. MiniMax M3's MSA architecture is a natural example for the KV cache variants notebook; DeepSeek R1-0528's extended thinking is a case study for inference-time scaling.

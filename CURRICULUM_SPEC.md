...
> **Current status:** The repository ships **64 of 70** notebooks.
...
**Model shortlist** (all Colab-T4 feasible in fp16): `HuggingFaceTB/SmolLM2-135M`, `SmolLM2-360M-Instruct`, `Qwen/Qwen2.5-0.5B[-Instruct]`, `Qwen/Qwen2.5-1.5B`, `Qwen/Qwen2.5-Coder-0.5B-Instruct`, `Qwen/Qwen3-0.6B` (thinking-capable), `Qwen/Qwen3-1.7B` (hybrid thinking/non-thinking via `/think` and `/no_think` prompt prefixes; use `/no_think` for latency-sensitive benchmarks), `Qwen/Qwen3.5-0.6B` (latest Qwen3.5 small variant), `meta-llama/Llama-3.2-1B[-Instruct]`, `TinyLlama/TinyLlama-1.1B-Chat-v1.0`, `microsoft/Phi-3.5-mini-instruct`, `google/gemma-3-1b-it`, `google/gemma-4-2b-it` (multimodal). Larger models (Qwen2.5-3B judge, DeepSeek-R1-Distill-Qwen-1.5B for reasoning demos, `meta-llama/Llama-4-Scout-17B-16E` and `Qwen/Qwen3-30B-A3B` for MoE notebooks — both require A100/H100 or quantization, DeepSeek-V4-Flash at 13B active params for MoE demos) are invoked only where strictly needed and marked as such. Llama-4 and Qwen3/Qwen3.5 variants are preferred for new notebooks targeting v0.2 due to their native MoE and thinking-token support.

**Framework version pins (pins in `pyproject.toml` and `environment.yml` are authoritative and may drift as the ecosystem moves):** `torch==2.7.*` (2.9 for FSDP2 notebooks), `triton==3.3.*`, `transformers==4.52.*` (RAG notebooks pin lower for `sentence-transformers` / `pylate` compatibility — see Spec-level risks), `vllm==0.22.*` (v0.22.1; Model Runner V2 — enabled via `VLLM_USE_V2_MODEL_RUNNER=1` — replaces CPU PyTorch ops with GPU-native Triton kernels, delivering 56% higher throughput on GB200; v0.22 adds Rust frontend preview, 28.9% latency gain via Cutlass FP8, multi-tier KV offload, DeepSeek V4 Pro/Flash NVFP4 fused MoE; v0.22.1 adds JetBrains Mellum v2 and zentorch AMD CPU quant; requires C++20 build toolchain; Transformers v4 deprecated in v0.22; NVIDIA Dynamo backend available as optional extra), `sglang==0.5.*` (v0.5.12.post1; HiCache UnifiedRadixTree + SSD offload via Mooncake, EAGLE-3 speculative decoding, DeepSeek V4 / Ring-2.6-1T / Gemma 4 support; post1 restores B200/B300 DeepSeek V4 accuracy; XGrammar-2 ~80× faster grammar compilation and ~3× faster constrained decoding vs vLLM; achieved 25× inference performance on GB300 NVL72), `xgrammar==0.2.*` (XGrammar-2; Structural Tag protocol unifies tool calling, reasoning channels, and custom output formats; integrated into vLLM, SGLang, and TensorRT-LLM), `sentence-transformers==5.4.1`, `faiss-cpu==1.13.2`, `peft==0.14.*` (DoRA supported ≥ 0.10), `trl==0.26.*`, `bitsandbytes==0.45.*`, `llmcompressor==0.10.*`, `langgraph==1.4.*`, `dspy-ai==3.0.*`, `autogen-agentchat==0.4.*` (Microsoft merged AutoGen with Semantic Kernel into **Microsoft Agent Framework 1.0** in 2026; AutoGen 0.4 enters maintenance mode; community fork continues as AG2 at `ag2ai/ag2`), `microsoft-agent-framework==1.0.*` (production successor to AutoGen 0.4 + Semantic Kernel), `crewai==1.12.*`, `mcp==1.6.*` (MCP 2026 spec RC — stateless Streamable HTTP core, Tasks extension, MCP Apps), `openai-agents==0.1.*` (OpenAI Agents SDK), `google-adk==1.0.*`, `pydantic-ai==0.1.*`, `lm-eval==0.4.15`, `inspect-ai==0.3.210`, `lighteval==0.11.0`, `ragas==0.2.14` (API-stable; 0.4.x available but breaks the 0.2.x metric API used in 02_rag/09 — upgrade is a v0.2 stretch goal), `jax==0.6.*`. **Note:** HuggingFace TGI moved to maintenance mode in 2025; vLLM and SGLang are the recommended replacements for production inference serving.
...
The dominant architecture shift since v0.1 is that MoE is now the default at scale: DeepSeek V4 Pro (1.6T total / 49B active, 1M-token context), Llama 4 Maverick (400B / 17B active, MoE-first in the Llama line), Qwen3.5-397B-A17B (flagship reasoning/math MoE), and Gemma 4 (multimodal) all use sparse MoE. DeepSeek V4 Flash (284B / 13B active) is a cost-efficient open MoE option for demo notebooks requiring larger active param counts than Qwen3-0.6B. Notable frontier-class open models released in 2026 that further close the open/closed gap: **Kimi K2.6** (Moonshot AI; 1T/32B active MoE, 80.2% SWE-bench Verified, Agent Swarm system scaling to 300 domain-specialized sub-agents across 4,000 coordinated steps), **Qwen 3.6 Plus** (Alibaba; 1M-token context, 78.8% SWE-bench Verified, leads Terminal-Bench 2.0), **GLM-5.1** (Z.AI; 754B/MoE, 58.4% SWE-bench Pro), **MiMo-V2.5** (Xiaomi; 310B/15B active MoE, 48T tokens, 1M-token context, fully open-sourced on HuggingFace, frontier-level multimodal agentic capability), and **MiniMax M3** (MiniMax; first open-weight model combining frontier coding + 1M context + native multimodal input; MSA sparse attention, 59.0% SWE-bench Pro, 70.06% OSWorld-Verified). **DeepSeek R1-0528** is a major reasoning upgrade: AIME 2025 70% → 87.5%, 2× deeper thinking (23K vs 12K avg tokens), 45–50% hallucination reduction. Apache 2.0 has become the dominant permissive license for 2026 open models (Gemma 4, Qwen 3.5, Mistral Large 3, Kimi K2.6); DeepSeek V4 ships MIT.

**Google I/O 2026** introduced new proprietary API models relevant to the production patterns track: **Gemini 3.5 Flash** combines frontier-level agentic and coding accuracy (Terminal-Bench 2.1: 76.2%, MCP Atlas: 83.6%) with Flash-class speed (~4× faster output than peer frontier models) and a 1M-token context window; pricing $1.50/$9.00 per 1M tokens in/out, $0.15 cached-read. **Gemini Spark** is a new persistent 24/7 agent product built on Gemini 3.5 and the Antigravity framework, able to execute long-horizon tasks autonomously on Google Cloud VMs. **Google ADK v1.0** reached stable release across Python, Go, Java, and TypeScript, and the **A2A v1.0 protocol** is now in production at 150+ organisations following its Linux Foundation adoption.
...
- **PegaFlow external KV cache** (Novita AI): A Rust-core KV cache storage engine that offloads GPU KV state to host memory or SSD and shares it across nodes via RDMA, integrating with vLLM and SGLang as a drop-in connector. Relevant extension to notebook 01_inference/06 (radix cache) and 05_serving/10 (disaggregated serving) — demonstrates production-grade KV offloading beyond the toy `SharedMemory` approach.
- **VeriCache** (arXiv 2605.17613): Compressed KV drafts tokens while full-precision KV on CPU/disk verifies — achieves lossless output quality at compressed-cache throughput. Natural extension to the KV compression notebooks (05_serving/03, 05_serving/04) and the speculative decoding notebook (01_inference/07).
...
- **MCP 2026 specification RC**: stateless Streamable HTTP core, SEP-2549 TTL caching, Tasks extension, MCP Apps, OAuth/OIDC hardening. The MCP server/client notebook (04_agents/05 and 08_production/06) should be updated to reference the RC spec; the final spec may require minor API changes once published.
- **MiniMax M3 / DeepSeek R1-0528**: New open-weight models to document in the model registry and GPU pricing notebook. MiniMax M3's MSA architecture is a natural example for the KV cache variants notebook; DeepSeek R1-0528's extended thinking is a case study for inference-time scaling.
...
(2025, trained from scratch with ternary weights {-1,0,+1}) achieves
...
**RAGAS 0.4.3** broke the 0.2.x metric API used in notebook
...
(GTC 2025) is now an optional backend for disaggregated serving that

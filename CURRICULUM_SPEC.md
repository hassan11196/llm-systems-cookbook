...
**Model shortlist** (all Colab-T4 feasible in fp16): `HuggingFaceTB/SmolLM2-135M`, `SmolLM2-360M-Instruct`, `Qwen/Qwen2.5-0.5B[-Instruct]`, `Qwen/Qwen2.5-1.5B`, `Qwen/Qwen2.5-Coder-0.5B-Instruct`, `meta-llama/Llama-3.2-1B[-Instruct]`, `TinyLlama/TinyLlama-1.1B-Chat-v1.0`, `microsoft/Phi-3.5-mini-instruct`, `Qwen/Qwen3-1.7B`. Larger models (Qwen2.5-3B judge, Llama-4-Scout-17B-16E, Qwen3-30B-A3B for MoE notebooks) are invoked only where strictly needed and marked as such. Qwen3-1.7B supports hybrid thinking/non-thinking modes via `/think` and `/no_think` prompt prefixes; use `/no_think` for latency-sensitive benchmarks.

**Framework version pins (last validated May 2026; pins in `pyproject.toml` and `environment.yml` are authoritative and may drift as the ecosystem moves):** `torch==2.7.*` (2.9 for FSDP2 notebooks), `triton==3.3.*`, `transformers==4.52.*`, `vllm==0.8.*` (V2 engine, async-first scheduler), `sglang==0.4.*` (RadixAttention, async constrained decoding), `sentence-transformers==5.4.1`, `faiss-cpu==1.13.2`, `peft==0.14.*`, `trl==0.26.*`, `bitsandbytes==0.45.*`, `llmcompressor==0.10.*`, `langgraph==1.4.*`, `dspy-ai==3.0.*`, `autogen-agentchat==0.4.*` (AG2 fork at `ag2ai/ag2`), `crewai==1.12.*`, `mcp==1.6.*`, `google-adk==1.0.*`, `pydantic-ai==0.1.*`, `lm-eval==0.4.15`, `inspect-ai==0.3.210`, `lighteval==0.11.0`, `ragas==0.2.14` (API-stable; 0.4.x available but breaks the 0.2.x metric API), `jax==0.5.*`. **Note:** HuggingFace TGI moved to maintenance mode in 2025; vLLM and SGLang are the recommended replacements for production inference serving.
...
Several topics are intentionally absent and worth flagging for a v2. **Multimodal** (vision encoders, speech, audio/video tokenization) gets no notebook; a natural addition is a `09_multimodal/` track with CLIP dual-encoder retrieval, LLaVA-style vision-language fine-tune, Whisper streaming inference, and a ScreenAgent notebook using the computer-use API pattern. **Safety and red-teaming** (jailbreak evals, constitutional AI, watermarking) is also absent. **Embedding training** (contrastive loss, hard negative mining, Matryoshka) is referenced via stretch goals but not taught directly. **Reasoning models** (o-series / R1-style GRPO training) are partially covered in notebook 03_training/08 (GRPO); a dedicated reasoning notebook benchmarking inference-time scaling laws ("thinking budget" control via BudgetThinker-style control tokens, arXiv 2508.17196) would be a natural addition for v0.3.

New developments to incorporate in v0.2:
...
- **RAGAS 0.4.3 (Jan 2026)** broke the 0.2.x metric API; the spec pins 0.2.14 deliberately and flags the upgrade as a stretch goal.
- **FSDP2's `fully_shard`** has no `auto_wrap_policy` / `use_orig_params` kwargs — implementers porting from FSDP1 tutorials will hit errors if they pass these.

...
**03 HumanEval with unbiased pass@k** (T4, 35 min). `pass_at_k(n, c, k) = 1 − C(n-c, k)/C(n, k)` with numerical stability; `unsafe_execute` sandbox via `multiprocess.Process` + `resource.setrlimit(RLIMIT_AS)` + timeout-kill. n=20 samples × T ∈ {0.2, 0.8}. Unit test: n=20,c=5,k=1 → 0.25 ± 0.01. **Qwen2.5-Coder-0.5B pass@1 at T=0.2 ≥ 0.20**; T=0.8 pass@10 ≥ T=0.2 pass@10; infinite-loop killed within timeout+1s. **Papers:** 2107.03374.

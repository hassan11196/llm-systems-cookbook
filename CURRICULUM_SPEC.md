...
- **MCP 2026-07-28 spec finalized**: the largest MCP revision since launch — stateless protocol core (no `Mcp-Session-Id`), `initialize`/`initialized` handshake removed, authorization rewritten around OAuth/OIDC RFCs, and a new reverse-DNS extensions framework carrying the Tasks extension and MCP Apps. Notebook 04_agents/05 (MCP server + client) and 08_production/06 (real MCP server) should migrate their handshake and session-handling code off the pre-2026-07-28 assumptions.

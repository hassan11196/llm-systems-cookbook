...
```{admonition} What's new in this revision
:class: note
...
- **Framework pins** refreshed to the current ecosystem (torch 2.7, vLLM
...
- **Google I/O 2026:** Gemini 3.5 Flash GA — frontier Flash-tier
  ...
  150+ organisations.
- **OpenAI:** GPT-5.5 Instant is now the default ChatGPT model for
...
- **vLLM v0.22:** Rust frontend preview (DP Supervisor), 28.9%
  ...
  + piecewise CUDA graphs; the v0.22.1 patch adds JetBrains
  Mellum v2 support, zentorch AMD Zen CPU quantized linear inference, and a
  DeepSeek-V4 init fix. **SGLang v0.5.12.post1:** HiCache
...
- **New frontier models (2026):** DeepSeek V4 Pro (1.6T/49B active
  ...
  model Intelligence Index v4.0); **MiniMax M3** (first
...
- **DeepSeek R1-0528:** major reasoning upgrade — AIME 2025 accuracy
  70% → 87.5%, avg thinking depth 12K → 23K tokens, 45-50% hallucination
  reduction, improved function calling.
- **Google TPU 8t/8i (Cloud Next 2026):** First bifurcated TPU
...
- **MCP 2026 RC:** stateless-core specification via Streamable HTTP published;
  ...
  server-rendered UIs, OAuth/OIDC hardening.
- **Serving infrastructure:** PegaFlow (Novita AI) — GIL-free Rust
...
**Microsoft Agent Framework 1.0** (the AutoGen + Semantic Kernel
merger).
